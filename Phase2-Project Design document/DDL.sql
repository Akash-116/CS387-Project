...
create table orders
    ( order_id serial, c_id int not null, area_id int, table_id int, dat date not null, received_time time, finished_time time, delivered_time time, delivery_person int, status text check (status in ('Preparing','Out for delivery','Delivered','Served')), order_type text check (order_type in ('Online','Dine')), primary key(order_id),
